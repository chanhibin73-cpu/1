    ...
    for model_name in models_to_try:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=90) as response:
                ...
                print(f"  ✓ {model_name} での要約に成功しました！")
                return res_json['candidates'][0]['content']['parts'][0]['text'], ""
                
        except urllib.error.HTTPError as e:
            err_msg = e.read().decode('utf-8', errors='ignore')
            print(f"  【スキップ】{model_name} がエラー (HTTP {e.code}) を返しました。次のモデルを試します。")
            last_error_msg = f"{model_name} (HTTP {e.code}): {err_msg}"
            continue # 次のモデルへ移行
            
        except Exception as e:
            print(f"  【スキップ】{model_name} で通信エラーが発生しました。次のモデルを試します。")
            last_error_msg = f"{model_name} (Network Error): {str(e)}"
            continue # 次のモデルへ移行

    # 全てのモデルで失敗した場合
    return None, f"指定された全てのモデルが使用不可またはエラーでした。最後のエラー:\n{last_error_msg}"
