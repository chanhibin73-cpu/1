...
def fetch_news():
    ...
    try:
        ...
        print(f"【警告】Googleニュースにアクセスをブロックされました (HTTP {e.code})。")
        return ""
    except Exception as e:
...
【期待するJSON】\n{{\n  "title": "見出し",\n  "content": "詳しい要約本文（内容の解説）",\n  "isBreaking": false\n}}\n【ニュース情報】\n{raw_news}"""
    ...
    try:
        ...
        if e.code in [404, 400]:
            print(f"【お知らせ】API側で {primary_model} が未提供またはエラー（{e.code}）のため、自動的に安定版 ({fallback_model}) に切り替えます。\n詳細: {error_body}")
            try:
                ...
                error_body2 = e2.read().decode('utf-8', errors='ignore')
                # ここでプログラムを止めず、Noneを返して「代替記事」を発行させます
                print(f"【重大エラー】予備モデルでの要約にも失敗 (HTTP {e2.code}):\n{error_body2}")
                return None
...
def main():
    ...
    if raw_news:
        ...
        if gemini_text:
            ...
            # ★【重要】AIの要約APIがエラーになっても、取得したニュースをそのまま記事として発行する！
            print("【警告】AI要約に失敗したため、生のニュースデータをそのまま記事として発行します。")
            fallback_content = "【要約AIで一時的なエラーが発生しています。以下の生データをお届けします】\n\n" + clean_html(raw_news[:1000])
            article_data = {"title": "最新の入試動向（AI要約エラー）", "content": fallback_content, "isBreaking": False}
    
    if not article_data:
        if is_first_issue:
            article_data = {"title": "入試日報新聞 創刊号", "content": "本紙は毎週火・木・土・日の朝9時に、入試情報を深く要約してお届けします。現在新しい情報を収集中です。", "isBreaking": False}
        else:
