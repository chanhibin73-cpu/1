...
【期待するJSON】\n{{\n  "title": "見出し",\n  "content": "要約本文",\n  "isBreaking": false\n}}\n【ニュース情報】\n{raw_news}"""
    ...
    data = {
        ...
        "systemInstruction": {"parts": [{"text": "あなたは入試新聞の編集長です。必ず正しいJSONのみを返してください。"}]},
...
def main():
    ...
    if raw_news:
        ...
        if gemini_text:
            ...
            article_data = {"title": "最新の入試動向（AI要約エラー）", "content": "要約AIでエラーが発生しました。生データ:\n" + clean_html(raw_news[:1000]), "isBreaking": False}
    
    if not article_data:
        article_data = {"title": "入試日報新聞 発行テスト", "content": "これは強制発行テストです。この記事が見えていればプログラムとデータベースは正常に接続されています！", "isBreaking": True}

    # ★今回は「スキップ」の処理をすべて削除しました。どんな結果であれ必ず発行します！
