...
def fetch_news():
    ...
    # ★ 修正ポイント：「when:30d」を追加し、過去1ヶ月以内のニュースに限定！
...
def main():
    ...
    # ★ 修正ポイント：毎日 11時台 を定例時間として扱う
    is_regular_time = (11 <= now_jst.hour <= 12)
    ...
    if not article_data:
        if is_first_issue:
            article_data = {"title": "入試日報新聞 創刊号", "content": "毎日午前11時に、過去1ヶ月の入試情報を深く要約してお届けします。現在新しい情報を収集中です。", "isBreaking": False}
        else:
    ...
    if not is_first_issue:
        if is_regular_time and has_today_regular and not is_breaking:
            print("✓ 本日の午前11時の定例号は発行済みです。スキップします。")
            return
    ...
    publish_date_str = f"{today_str} {'午前11時発行' if is_regular_time else now_jst.strftime('%H時%M分発行')} {issue_label}"
