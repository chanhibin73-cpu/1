...
def fetch_news():
    ...
    try:
        ...
        print(f"【重大エラー】ニュース自動検索失敗: {e}")
        sys.exit(1)

def get_gemini_summary(raw_news, api_key, is_regular_time):
    print("-> Gemini API (3.6flash) によるニュース自動要約を実行中...")
    
    # ユーザー指定のモデルを確実に使用（変更しません）
...
def main():
    ...
    # .strip() を追加し、見えない改行や空白が混じっていても取り除くように対策
    ...
    if not api_key:
        print("【重大エラー】APIキー (GEMINI_API_KEY) が設定されていません。")
        sys.exit(1)
        
    if not firebase_cert_json:
        print("【重大エラー】Firebase秘密鍵 (FIREBASE_SERVICE_ACCOUNT) が設定されていません。")
        sys.exit(1)

    try:
        # Firebaseの鍵データ内に不正な改行文字列が含まれている場合の改行エラー対策
        ...
        print(f"【重大エラー】Firebase初期化失敗。改行エラーまたはJSON形式の間違いです: {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"【重大エラー】Firestoreから既存記事の読み込み失敗: {e}")
        sys.exit(1)
    ...
    if raw_news:
        ...
        if gemini_text:
            # AIの回答に含まれる「改行エラー」を無効化する強力な処理を追加
            cleaned = gemini_text.replace("```json", "").replace("```", "").strip()
            try:
                # 1. まず普通に読み込みを試みる (strict=Falseで多少の改行を許容)
                article_data = json.loads(cleaned, strict=False)
            except Exception as e1:
                print(f"【警告】AI回答のJSONパースで改行エラー発生(1回目): {e1}")
                try:
                    # 2. 本文内の生の改行コードを安全な文字列「\\n」に強制置換してエラーを回避
                    cleaned_escaped = cleaned.replace('\n', '\\n').replace('\r', '')
                    article_data = json.loads(cleaned_escaped, strict=False)
                    print("-> 改行エラーを自動修復し、データの読み込みに成功しました。")
                except Exception as e2:
                    print(f"【警告】改行の修復に失敗したため代替フォーマットを使用します: {e2}")
    ...
    if not article_data:
        if is_first_issue:
            ...
            print("✓ 取得できたニュースがなく、創刊号でもないため終了します。")
            return
    ...
    try:
        ...
        print(f"【重大エラー】Firestoreへの書き込み（保存）に失敗しました: {e}")
        sys.exit(1)
