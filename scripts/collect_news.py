...
def main():
    print("=== 入試ニュース収集プログラム（号数管理・速報抑制・1日1回版）開始 ===")
    ...
    if not raw_news and is_first_issue:
        ...
        # ★改善ポイント：AIエラー時の保険として、「取得中です」ではなく実際の生のニュースを入れておく
        fallback_content = raw_news[:450] + "..." if raw_news else "現在新しい入試関連のニュースはありません。"
    ...
    # 「速報ではない」かつ「本日の定例号がすでに発行済み」の場合はスキップ
