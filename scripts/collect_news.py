...
if not GEMINI_API_KEY or not FIREBASE_SERVICE_ACCOUNT_JSON:
    raise ValueError("必要な環境変数が設定されていません (GEMINI_API_KEY, FIREBASE_SERVICE_ACCOUNT_JSON)")
...
# --- 3. Gemini (gemini-3.6-flash) による新聞記事化 ---
...
def save_to_firestore(article_data):
    ...
    app_id = os.environ.get("APP_ID", "default-exam-news-app")
...
def main():
    ...
    if not raw_news:
        print("有効なニュースデータがありませんでした。")
        return
        
    print("Gemini (gemini-3.6-flash) による要約・記事編集を開始します...")
    article = generate_newspaper_article(raw_news)
    print(f"生成タイトル: {article['title']}")
