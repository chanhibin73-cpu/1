...
【期待するJSON】\n{{\n  "title": "見出し",\n  "content": "詳しい要約本文（内容の解説）",\n  "isBreaking": false\n}}\n【ニュース情報】\n{raw_news}"""
    ...
    # AIがパニックを起こさないよう、リクエスト構造をシンプルにしました
    data = {
        "contents": [{"parts": [{"text": prompt}]}],
        "systemInstruction": {"parts": [{"text": "あなたは大学入試専門の新聞編集長です。必ずJSON形式のみを出力してください。"}]}
    }
    ...
    try:
        ...
        if e.code in [404, 400]:
            ...
            try:
                ...
                print(f"【警告】予備モデルでの要約にも失敗 (HTTP {e2.code}): {e2.read().decode('utf-8')}")
                return None # ここでエラーにせず「None（要約失敗）」を返して続行させる
            except Exception as e2:
                print(f"【警告】予備モデルでの要約にも失敗: {e2}")
                return None
        else:
            print(f"【警告】Gemini API通信エラー ({e.code}): {error_msg}")
            return None
    except Exception as e:
        print(f"【警告】Gemini API要約失敗: {e}")
        return None
...
def main():
    ...
    if raw_news:
        ...
        if gemini_text:
            ...
            # ★【要約失敗時のフォールバック】AIが失敗しても、原文のまま記事を発行する
            article_data = {
                "title": "【要約AI 調整中】最新入試ニュース", 
                "content": "現在AI要約システムが混雑または調整中のため、ニュースの内容をそのままお届けします。\n\n" + raw_news[:1500], 
                "isBreaking": False
            }
