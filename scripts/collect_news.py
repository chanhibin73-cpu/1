...
【期待するJSON】\n{{\n  "title": "見出し",\n  "content": "詳しい要約本文（内容の解説）",\n  "isBreaking": false\n}}\n【ニュース情報】\n{raw_news}"""
    ...
    try:
        ...
        # もしGoogleサーバーから「そのモデルは存在しない(404)」と返ってきた場合は、自動で切り替える
        if e.code == 404:
            print(f"【お知らせ】API側で {primary_model} が未提供のため、自動的に安定版 ({fallback_model}) に切り替えて実行を継続します。")
            try:
...
def main():
    ...
    # Firestoreの保存先IDを直接固定し、「パスが空になる」エラーを完全に防止
