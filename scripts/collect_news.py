...
def check_existing_articles(project_id, app_id, today_str):
    url = f"[https://firestore.googleapis.com/v1/projects/](https://firestore.googleapis.com/v1/projects/){project_id}/databases/(default)/documents/artifacts/{app_id}/public/data/articles"
...
def main():
    ...
    # 定例日時の判定 (火=1, 木=3, 土=5, 日=6 の 12時台〜13時台)
    # ※GitHub Actionsの起動遅延を考慮して12:00〜13:59を定例時間帯とする
    ...
    if not raw_news and is_first_issue:
        ...
        if raw_news:
            ...
            if gemini_text:
                try:
                    # Markdown記法 (```json など) を強固に除去しJSONパースエラーを防ぐ
    ...
    if is_regular_time:
        ...
        # 定例時間以外は速報チェックモード。AIが速報と判定しなかった場合はスキップ
    ...
    payload = {
        "fields": {
            ...
            "content": {"stringValue": content_text},
