...
def main():
    ...
    if not raw_news and is_first_issue:
        ...
        # AIからテキストが返ってこなかった場合は保存を中止
        if not gemini_text:
            print("【エラー】AIから要約記事が取得できませんでした。内容が不完全なため保存をスキップして終了します。")
            return
        ...
        try:
            ...
            print(f"【エラー】AIからの応答が正しいJSON形式ではありませんでした。内容が不完全なため保存をスキップして終了します。エラー詳細: {e}")
            return
            
        # タイトルや本文が空っぽの場合は保存を中止
        if not article_data.get("title") or not article_data.get("content"):
            print("【エラー】生成された記事のタイトルまたは本文が空です。内容が不完全なため保存をスキップして終了します。")
            return
