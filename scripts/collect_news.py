...
【期待するJSON】\n{{\n  "title": "見出し",\n  "content": "詳しい要約本文（内容の解説）",\n  "isBreaking": false\n}}\n【ニュース情報】\n{raw_news}"""
    ...
    try:
        ...
        if e.code == 404:
            print("💡ヒント: 「404 Not Found」エラーです。ご指定のモデル（gemini-3.6-flash）がAPI上に存在しないことが原因です。")
        sys.exit(1)
    except Exception as e:
...
def main():
    ...
    # ★★★ ここが一番重要な修正です ★★★
    # 環境変数からの読み込みをやめ、直接ハードコード（埋め込み）して絶対に空にならないようにしました
    app_id = "1:376024106499:web:108b0a027015c782c97036"
    ...
    try:
        ...
        print(f"【重大エラー】Firebase初期化失敗。改行エラーまたはJSON形式の間違いです:\n{e}")
        traceback.print_exc()
        sys.exit(1)
    ...
    try:
        ...
        print(f"【重大エラー】Firestoreから既存記事の読み込み失敗:\n{e}")
        traceback.print_exc()
        sys.exit(1)
    ...
    if raw_news:
        ...
        if gemini_text:
            ...
            try:
                ...
                print(f"【警告】AI回答のJSONパースで改行エラー発生: {e1}")
                try:
    ...
    try:
        ...
        print(f"【重大エラー】Firestoreへの書き込み（保存）に失敗しました:\n{e}")
        traceback.print_exc()
        sys.exit(1)
