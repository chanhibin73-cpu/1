...
def get_gemini_response(prompt, api_key):
    ...
    data = {
        ...
        "systemInstruction": {
            "parts": [{"text": "あなたは大学入試専門の新聞編集長です。提供されたニュースの内容をしっかりと読み込み、URLやリンクは一切含めず、受験生向けに分かりやすくまとめた要約記事を作成してください。同じような内容の重複があっても構いません。本当に重大な変更や緊急ニュース（試験の延期や大幅な制度変更など）の場合のみisBreakingをtrueにしてください。それ以外はfalseにしてください。"}]
        },
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            for item in root.findall(".//item")[:8]:
                ...
                # URLやHTMLタグを除去して純粋なテキストのみにする
                clean_desc = re.sub(r'http\S+', '', desc)
                texts.append(f"・{title}: {clean_desc}")
...
def main():
    print("=== 入試ニュース収集プログラム（火木土日・要約徹底版）開始 ===")
